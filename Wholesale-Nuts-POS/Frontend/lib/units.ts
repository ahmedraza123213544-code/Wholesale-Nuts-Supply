...
export function resolveItemUnit(item: unknown): string {
  const rec = (item ?? {}) as Record<string, unknown>;
  const product = rec.product as Record<string, unknown> | undefined;
  const productUnit = product?.unit as Record<string, unknown> | undefined;
  const itemUnit = rec.unit as Record<string, unknown> | string | undefined;

  return (
    (productUnit?.name as string) ||
    (product?.unit_name as string) ||
    (product?.unitName as string) ||
    (typeof itemUnit === "object" ? (itemUnit?.name as string) : "") ||
    (typeof itemUnit === "string" ? itemUnit : "") ||
    (rec.unit_name as string) ||
